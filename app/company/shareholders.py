...
def shareholders(company, page_title):
    """株主/社員情報の一覧ページ"""
    shareholder_list = _shareholders.get_shareholders_by_company(company.id)
    # 主たる株主（親がいない）を先に計算してテンプレートへ安全に引き渡す
    main_shareholders = [s for s in shareholder_list if s.parent_id is None]
    raw_totals_map = _shareholders.compute_group_totals_both_map(company.id)
...
def register_main_shareholder(company, page_title):
    ...
    if form.validate_on_submit():
        new_shareholder, error = _shareholders.add_shareholder(company.id, form)
        if error:
            flash(error, 'warning')
    ...
    main_shareholders = _shareholders.get_main_shareholders(company.id)
...
def register_related_shareholder(company, main_shareholder_id, page_title):
    """特殊関係人の新規登録"""
    main_shareholder = _shareholders.get_shareholder_by_id(main_shareholder_id)
    form = RelatedShareholderForm(request.form)
    form.officer_position.choices = get_officer_choices(page_title)

    if form.validate_on_submit():
        new_shareholder, error = _shareholders.add_shareholder(company.id, form, parent_id=main_shareholder_id)
        if error:
            flash(error, 'warning')
            return redirect(url_for('company.shareholders'))
    ...
    group_number = _shareholders.get_main_shareholder_group_number(company.id, main_shareholder_id)
    navigation_state = get_navigation_state('shareholders')
...
def edit_shareholder(company, shareholder_id, page_title):
    """株主情報の編集"""
    shareholder = _shareholders.get_shareholder_by_id(shareholder_id)
    form_class = _shareholders.get_shareholder_form(shareholder)
    ...
    if request.method == 'GET' and shareholder.parent_id is not None and hasattr(form, 'is_address_same_as_main'):
        ...
        same = _shareholders.is_same_address(shareholder, shareholder.parent) if shareholder.parent is not None else False
        form.is_address_same_as_main.data = same

    if form.validate_on_submit():
        _shareholders.update_shareholder(shareholder_id, form)
...
def delete_shareholder(company, shareholder_id, page_title):
    """株主/社員の削除"""
    _shareholders.delete_shareholder(shareholder_id)
